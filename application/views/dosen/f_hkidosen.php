...
<form action="<?php echo base_URL(); ?>index.php/dosen/dosenhki/<?php echo $act; ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
	...
	?>
	<tr><td width="20%">Judul Penelitian</td><td><b><input type="text" name="judul" required value="<?php if($data->num_rows()>0){ echo $data->row()->judulpenelitian; } ?>" style="width: 700px" class="form-control" autofocus></b>
	<input type="hidden" name="idp" value="<?php if($data->num_rows()>0){ echo $data->row()->idpenelitian; } ?> ">
	</td></tr>
	...
	<tr><td width="20%">Jenis HKI</td><td><b><select type="text" name="jenis" required value="<?php echo $jenis; ?>" style="width: 700px" class="form-control">
		<option value="" selected>-- Pilih --</option>
		<option value="" >Paten</option>
		<option value="" >Paten Sederhana</option>
		<option value="" >Hak Cipta</option>
		<option value="" >Merk Dagang</option>
		<option value="" >Rahasia Dagang</option>	
		<option value="" >Desain Produksi Industri</option>
		<option value="" >Indikasi Geografis</option>
		<option value="" >Perlindungan Varietas Tanaman</option>
		<option value="" >Perlindungan Topografi Sirkuit Terpadu</option>
	...
	<a href="<?php echo base_URL(); ?>index.php/dosen/detail_hki/<?php echo $this->session->userdata('idpenelitian'); ?>" class="btn btn-success"><i class="icon icon-arrow-left icon-white"></i> Kembali</a>
	</table>
</form>
</div>
