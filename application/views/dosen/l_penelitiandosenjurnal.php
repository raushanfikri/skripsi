<div class="clearfix">
	...
	<table class="table table-bordered table-hover" id="table">
		...
		/*if (empty($data)) {
			...
			foreach ($data as $b) {
				$get_personil = $this->db->select("*")->from("penelitian")->join("detail_anggotapenelitian",
				"detail_anggotapenelitian.idpenelitian=penelitian.idpenelitian")->join("dosen",
				"dosen.nidn=detail_anggotapenelitian.nidn")->where("penelitian.idpenelitian",$b->idpenelitian)->get()->result();
		?>
			<tr>
			<td style="font-size: 11px"><?php echo $no; ?></td>
				...
				<td class="ctr">
					<a href="<?= base_url('index.php/dosen/detail_jurnal/') . '/' . $b->idpenelitian;?>"
						class="btn btn-primary btn-sm" title="Tambah Data Publikasi"><i class="icon-list icon-white">
						</i></a>
